...
export class RandomTeams {
	...
	randomGGFactorySets: {[format: string]: {[species: string]: BattleFactorySpecies}} = require('./gg-factory-sets.json');

	randomGGFactorySet(
		species: Species, teamData: RandomTeamsTypes.FactoryTeamDetails, isGod: string, slot?: string
	): RandomTeamsTypes.RandomFactorySet | null {
		const id = toID(species.name);
		console.log(id);
		let setList;
		if (isGod === "Gods") {
			setList = this.randomGGFactorySets[isGod][id].sets;
		} else {
			console.log(slot);
			setList = this.randomGGFactorySets[isGod][slot][id].sets;
		}
		...
		const ability = this.sampleIfArray(setData.set.ability);
		const nature = this.sampleIfArray(setData.set.nature);
		const level = this.adjustLevel || setData.set.level;
	...
	randomGGFactoryTeam(side: PlayerOptions, depth = 0): RandomTeamsTypes.RandomFactorySet[] {

		const forceResult = (depth >= 4);
		// Leaving Monotype code in comments in case it's used in the future
		// const isMonotype = !!this.forceMonotype || this.dex.formats.getRuleTable(this.format).has('sametypeclause');

		// The teams generated depend on the god choice in such a way that
		// no exploitable information is leaked from rolling the tier in getTeam(p1).
		const stats = ["hp", "atk", "def", "spa", "spd", "spe"];
		const god = this.sample(Object.keys(this.randomGGFactorySets["Gods"]));
		const deadslot = this.sample(this.randomGGFactorySets["Gods"][god].deadslots);
		const godStats = Dex.species.get(god).baseStats;
		...
		let curMon = -1;
		const receiverpool = this.randomGGFactorySets["Receivers"];
		while (curMon < 6) {
			console.log(curMon);
			const curSlot = stats[curMon];
			console.log(curSlot);
			if (curSlot === deadslot) {
				curMon++;
				continue;
			...
			let set = false;
			if (curMon < 0) {
				species = Dex.species.get(god);
				set = this.randomGGFactorySet(species, teamData, "Gods");
				species = Dex.species.get(set.species);
			} else {
				species = this.dex.species.get(this.sampleNoReplace(Object.keys(receiverpool[curSlot])));
				if (!species.exists) continue;
				if (species.baseStats[curSlot] >= godStats[curSlot]) continue;
				// Limit to one of each species (Species Clause)
				if (teamData.baseFormes[species.baseSpecies]) continue;
				set = this.randomGGFactorySet(species, teamData, "Receivers", curSlot);
			}
				if (!set) continue;
				...
				// Okay, the set passes, add it to our team
				if (curMon >= 0 && curMon <= stats.indexOf(deadslot)) {
					pokemon.splice(pokemon.length - 1, 0, set);
				} else {
					pokemon.push(set);
				}
		...
		if (pokemon.length < this.maxTeamSize) return this.randomGGFactoryTeam(side, ++depth);

		// Quality control
		if (!teamData.forceResult) {
			for (const requiredFamily of requiredMoveFamilies) {
				if (!teamData.has[requiredFamily]) return this.randomGGFactoryTeam(side, ++depth);
			}
			for (const typeName in teamData.weaknesses) {
				if (teamData.weaknesses[typeName] >= 3) return this.randomGGFactoryTeam(side, ++depth);
			}
		}
