...
// 18/10/27 TrashChannel: Based on mixandmega/scripts.ts
...
export const Scripts: ModdedBattleScriptsData = {
	...
	actions: {
		...
		runMegaEvo(pokemon) {
			...
			this.battle.add('-start', pokemon, this.dex.generateMegaStoneName(bitchSpecies as string), '[silent]');
		...
		getMixedSpecies(originalForme, bitchForme) {
			...
			let originalSpecies = context.species.get(originalForme);
			let bitchSpecies = context.species.get(bitchForme);
		...
		doGetMixedSpecies(speciesOrForme, deltas) {
			...
			let context = (typeof Dex != 'undefined') ? Dex : this.dex;
			if (!speciesOrForme || typeof speciesOrForme === 'string') speciesOrForme = context.species.get(speciesOrForme);
